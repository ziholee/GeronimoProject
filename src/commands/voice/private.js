...
module.exports = {
	...
	async execute(interaction) {
		...
		if (channelInfo.ownerId !== interaction.user.id) {
			await interaction.reply({ content: '이 채널의 소유자만 비공개 설정을 변경할 수 있습니다.', ephemeral: true });
			return;
		}
		...
		try {
			if (isPrivate) {
				...
				await interaction.reply({ content: '채널이 비공개로 설정되었습니다. 이제 소유자만 다른 사용자를 초대할 수 있습니다.', ephemeral: true });
			}
			else {
				...
				await interaction.reply({ content: '채널이 공개로 설정되었습니다.', ephemeral: true });
			}
		}
		catch (error) {
			console.error('채널 비공개 설정 실패:', error);
			await interaction.reply({ content: '채널 비공개 설정 중 오류가 발생했습니다.', ephemeral: true });
		}
	},
};
